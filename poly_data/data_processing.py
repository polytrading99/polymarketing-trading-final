...
from app.services.persistence import persist_order_event
import poly_data.CONSTANTS as CONSTANTS
import poly_data.global_state as global_state
from poly_data.data_utils import set_order, set_position, update_positions
...
def process_price_change(asset, side, price_level, new_size, asset_id=None):
    ...
    if asset_id is not None and asset in global_state.all_data:
        if asset_id != global_state.all_data[asset].get('asset_id'):
            return
    if side == 'bids':
...
def process_data(json_datas, trade=True):
    ...
    for json_data in json_datas:
        ...
        if event_type == 'book':
            ...
            # Extract asset_id from json_data if available
            ...
            if row['event_type'] == 'trade':
                ...
                try:
                    for maker_order in row.get('maker_orders', []):
                        order_id = maker_order.get('id') or maker_order.get('order_id')
                        if not order_id:
                            continue

                        possible_side = maker_order.get('side', row['side'])
                        order_event = {
                            'market': row['market'],
                            'id': order_id,
                            'asset_id': token,
                            'side': str(possible_side).upper(),
                            'price': maker_order.get('price', row.get('price')),
                            'original_size': maker_order.get('size') or maker_order.get('original_size') or row.get('size'),
                            'size_matched': maker_order.get('matched_amount') or row.get('size'),
                            'status': row['status'],
                        }
                        persist_order_event(order_event)
                except Exception as exc:
                    print(f"Failed to persist trade event orders: {exc}")
                ...
                if row['status'] == 'CONFIRMED' or row['status'] == 'FAILED':
                    ...
                    if metrics_registry.trade_counter is not None:
                        try:
                            metrics_registry.trade_counter.labels(
                                market=market,
                                token=str(token),
                                side=side.upper(),
                            ).inc(abs(float(size)))
                        except Exception as exc:
                            print(f"Failed to record trade metric: {exc}")
                ...
                try:
                    persist_order_event(row)
                except Exception as exc:
                    print(f"Failed to persist order event: {exc}")
                asyncio.create_task(perform_trade(market))
